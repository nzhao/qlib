...
krL=0.5; thetaL=pi/3;phiL=pi/6;
krP=0.5; thetaP=0;phiP=0;
R = ott13.z_rotation_matrix(thetaL,phiL);%R = rotation_matrix([0,1,0],pi/3); %The same as zeng, rotate the sate.
D = ott13.wigner_rotation_matrix(Nmax,R);D=full(D.')% This is the Zeng's D
MP1 = spharm(n,m,thetaP,phiP)*(-1);MP1
ML1 = spharm(n,1,thetaL,phiL)*(-1);
ML0 = spharm(n,0,thetaL,phiL);
ML_1 = spharm(n,-1,thetaL,phiL)*(-1)^(-1);
...
m=[-1,0,1];%m=1;
MP1 = spharm(n,m,thetaL,phiL).*(-1).^m;
D3=sqrt(4*pi/(2*n+1))*conj(MP1);D3
D1(:,2)
