...
Mnew=Mnew*R;
[MP1;Mnew]
%Ey term has a -, why? may be the D is not the D in Mish, to test.
...
[EqL';EqR']'
alpha=phiL;% add m'*alpha to get D
Dmish=[D1(1,:)*exp(-1i*(-1)*alpha);D1(2,:)*exp(-1i*0*alpha);D1(3,:)*exp(-1i*1*alpha)];
D=full(Dmish)
